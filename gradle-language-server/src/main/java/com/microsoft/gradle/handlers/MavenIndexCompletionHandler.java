...
public class MavenIndexCompletionHandler {
  ...
  public MavenIndexCompletionHandler() {
    try {
      ...
      Map<String, Integer> artifactUsageDict = new Gson().fromJson(new JsonReader(new InputStreamReader(inputStream)),
  ...
  private List<CompletionItem> getArtifactIdCompletions(String groupId, String text, Range range) {
    ...
    return CompletionUtils.getArtifactIdCompletions(groupId, text, range, this.sortedRepository.get(groupId), sequence);
  }
}
